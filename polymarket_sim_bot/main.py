...
def _check_open_position_resolutions(state: SimulationState) -> None:
    ...
    for position_key in list(state.open_positions.keys()):
        ...
        try:
            markets = pm.get_all_markets(active=False, closed=True)
        except Exception as exc:
            print(f"[aviso] nao foi possivel checar resolucao de {market_id}: {exc}")
            continue

        match = next((m for m in markets if m.get("conditionId") == market_id
                      or m.get("id") == market_id), None)
        if not match:
            continue  # ainda nao resolvido (ou nao encontrado nesta pagina)
